...
impl<'conn, 'env, Params: Default + Serialize, Cols: Default + Serialize>
    Statement<'conn, 'env, Params, Cols>
{
    pub fn new(
        conn: &'conn Connection<'env>,
        stmt_str: &str,
    ) -> Result<Statement<'conn, 'env, Params, Cols>, SQLRETURN> {
        ...
        unsafe { bind_params(stmt, &params)? };

        let cols = Box::new(Default::default());
        unsafe { bind_cols(stmt, &cols)? };
...
impl<'conn, 'env, Params: Default + Serialize, Cols: Default + Serialize> Drop
    for Statement<'conn, 'env, Params, Cols>
{
    fn drop(&mut self) {
...
mod tests {
    ...
    fn exec_stmt() {
        ...
        let mut stmt: Statement<(i32, ()), (Nullable<i32>, ())> =
